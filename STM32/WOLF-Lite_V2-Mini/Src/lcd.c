...
static void LCD_displayTopButtons(bool redraw) { // display the top buttons
	...
	printInfoSmall(LAYOUT->TOPBUTTONS_PRE_X, LAYOUT->TOPBUTTONS_PRE_Y, LAYOUT->TOPBUTTONS_WIDTH, LAYOUT->TOPBUTTONS_HEIGHT, "DRV", COLOR->BUTTON_BACKGROUND, COLOR->BUTTON_TEXT,
	               COLOR->BUTTON_INACTIVE_TEXT, TRX.ADC_Driver);
	char buff[64] = {0};

	printInfoSmall(LAYOUT->TOPBUTTONS_ATT_X, LAYOUT->TOPBUTTONS_ATT_Y, LAYOUT->TOPBUTTONS_WIDTH, LAYOUT->TOPBUTTONS_HEIGHT, "ATT", COLOR->BUTTON_BACKGROUND, COLOR->BUTTON_TEXT,
	               COLOR->BUTTON_INACTIVE_TEXT, TRX.ATT);
	printInfoSmall(LAYOUT->TOPBUTTONS_PGA_X, LAYOUT->TOPBUTTONS_PGA_Y, LAYOUT->TOPBUTTONS_WIDTH, LAYOUT->TOPBUTTONS_HEIGHT, "SQL", COLOR->BUTTON_BACKGROUND, COLOR->BUTTON_TEXT,
	               COLOR->BUTTON_INACTIVE_TEXT, CurrentVFO->SQL);
	printInfoSmall(LAYOUT->TOPBUTTONS_DRV_X, LAYOUT->TOPBUTTONS_DRV_Y, LAYOUT->TOPBUTTONS_WIDTH, LAYOUT->TOPBUTTONS_HEIGHT, "RIT", COLOR->BUTTON_BACKGROUND, COLOR->BUTTON_TEXT,
	               COLOR->BUTTON_INACTIVE_TEXT, TRX.RIT_Enabled || TRX.XIT_Enabled || TRX.SPLIT_Enabled);
	printInfoSmall(LAYOUT->TOPBUTTONS_FAST_X, LAYOUT->TOPBUTTONS_FAST_Y, LAYOUT->TOPBUTTONS_WIDTH, LAYOUT->TOPBUTTONS_HEIGHT, "FAST", COLOR->BUTTON_BACKGROUND, COLOR->BUTTON_TEXT,
	               COLOR->BUTTON_INACTIVE_TEXT, TRX.Fast);
	printInfoSmall(LAYOUT->TOPBUTTONS_AGC_X, LAYOUT->TOPBUTTONS_AGC_Y, LAYOUT->TOPBUTTONS_WIDTH, LAYOUT->TOPBUTTONS_HEIGHT, "AGC", COLOR->BUTTON_BACKGROUND, COLOR->BUTTON_TEXT,
	               COLOR->BUTTON_INACTIVE_TEXT, CurrentVFO->AGC);
	if (CurrentVFO->DNR_Type == 1) {
		printInfoSmall(LAYOUT->TOPBUTTONS_DNR_X, LAYOUT->TOPBUTTONS_DNR_Y, LAYOUT->TOPBUTTONS_WIDTH, LAYOUT->TOPBUTTONS_HEIGHT, "DNR", COLOR->BUTTON_BACKGROUND, COLOR->BUTTON_TEXT,
		               COLOR->BUTTON_INACTIVE_TEXT, true);
	} else if (CurrentVFO->DNR_Type == 2) {
		printInfoSmall(LAYOUT->TOPBUTTONS_DNR_X, LAYOUT->TOPBUTTONS_DNR_Y, LAYOUT->TOPBUTTONS_WIDTH, LAYOUT->TOPBUTTONS_HEIGHT, "DNR2", COLOR->BUTTON_BACKGROUND, COLOR->BUTTON_TEXT,
		               COLOR->BUTTON_INACTIVE_TEXT, true);
	} else {
		printInfoSmall(LAYOUT->TOPBUTTONS_DNR_X, LAYOUT->TOPBUTTONS_DNR_Y, LAYOUT->TOPBUTTONS_WIDTH, LAYOUT->TOPBUTTONS_HEIGHT, "DNR", COLOR->BUTTON_BACKGROUND, COLOR->BUTTON_TEXT,
		               COLOR->BUTTON_INACTIVE_TEXT, false);
	}
	printInfoSmall(LAYOUT->TOPBUTTONS_NB_X, LAYOUT->TOPBUTTONS_NB_Y, LAYOUT->TOPBUTTONS_WIDTH, LAYOUT->TOPBUTTONS_HEIGHT, "NB", COLOR->BUTTON_BACKGROUND, COLOR->BUTTON_TEXT,
	               COLOR->BUTTON_INACTIVE_TEXT, TRX.NOISE_BLANKER);
	printInfoSmall(LAYOUT->TOPBUTTONS_MUTE_X, LAYOUT->TOPBUTTONS_MUTE_Y, LAYOUT->TOPBUTTONS_WIDTH, LAYOUT->TOPBUTTONS_HEIGHT, "MUTE", COLOR->BUTTON_BACKGROUND, COLOR_RED,
	               COLOR->BUTTON_INACTIVE_TEXT, TRX_AFAmp_Mute);
	printInfoSmall(LAYOUT->TOPBUTTONS_LOCK_X, LAYOUT->TOPBUTTONS_LOCK_Y, LAYOUT->TOPBUTTONS_WIDTH, LAYOUT->TOPBUTTONS_HEIGHT, "LOCK", COLOR->BUTTON_BACKGROUND, COLOR_RED,
...
static void LCD_displayBottomButtons(bool redraw) {
	...
	for (uint8_t i = 0; i < FUNCBUTTONS_ON_PAGE; i++) {
		printButton(curr_x, LAYOUT->BOTTOM_BUTTONS_BLOCK_TOP, LAYOUT->BOTTOM_BUTTONS_ONE_WIDTH, LAYOUT->BOTTOM_BUTTONS_BLOCK_HEIGHT,
		            (char *)PERIPH_FrontPanel_FuncButtonsList[TRX.FuncButtons[TRX.FRONTPANEL_funcbuttons_page * FUNCBUTTONS_ON_PAGE + i]].name, true, false, false, 0,
...
static void LCD_displayFreqInfo(bool redraw) { // display the frequency on the screen
	...
	if (redraw) {
		LCDDriver_Fill_RectWH(LAYOUT->FREQ_LEFT_MARGIN, LAYOUT->FREQ_Y_TOP - 10, LCD_WIDTH - LAYOUT->FREQ_LEFT_MARGIN - LAYOUT->FREQ_RIGHT_MARGIN, LAYOUT->FREQ_BLOCK_HEIGHT, BG_COLOR);
	}

	if ((mhz_x_offset - LAYOUT->FREQ_LEFT_MARGIN) > 0) {
		LCDDriver_Fill_RectWH(LAYOUT->FREQ_LEFT_MARGIN, LAYOUT->FREQ_Y_TOP - 10, mhz_x_offset - LAYOUT->FREQ_LEFT_MARGIN, LAYOUT->FREQ_BLOCK_HEIGHT, BG_COLOR);
	}
...
static void LCD_displayStatusInfoBar(bool redraw) {
	...
	//	sprintf(buff, "CPU:%d%% ",(uint32_t)CPU_LOAD.Load);
	...
	sprintf(buff, "%dkHz", FFT_current_spectrum_width_hz / 1000);
