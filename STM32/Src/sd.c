...
static void SDCOMM_EXPORT_SETT_handler(void) {
	...
	if (f_open(&File, "wolf.ini", FA_CREATE_ALWAYS | FA_WRITE) == FR_OK) {
		...
		if (res) {
			...
			SD_WRITE_SETT_LINE("TRX.Split_Mode_Sync_Freq", (uint32_t *)&TRX.Split_Mode_Sync_Freq, SYSMENU_BOOLEAN);
#if HRDW_HAS_DUAL_RX
			...
			SD_WRITE_SETT_LINE("CALIBRATE.ALC_Port_Enabled", (uint32_t *)&CALIBRATE.ALC_Port_Enabled, SYSMENU_BOOLEAN);
