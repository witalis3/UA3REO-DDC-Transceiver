...
static void SDCOMM_EXPORT_SETT(void)
{
	...
	if(f_open(&File, "wolf.ini", FA_CREATE_ALWAYS | FA_WRITE) == FR_OK)
	{
		...
		if(res) SD_WRITE_SETT_LINE("CALIBRATE.CIC_GAINER", (uint32_t*)&CALIBRATE.CIC_GAINER_val, SYSMENU_UINT8);
...
static void SDCOMM_PARSE_SETT_LINE(char* line)
{
	...
	if (strcmp(name, "CALIBRATE.CIC_GAINER") == 0) CALIBRATE.CIC_GAINER_val = (uint8_t)uintval;
