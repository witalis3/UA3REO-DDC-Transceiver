...
static void SDCOMM_EXPORT_SETT(void)
{
	...
	if(f_open(&File, "wolf.ini", FA_CREATE_ALWAYS | FA_WRITE) == FR_OK)
	{
		...
		if(res) SD_WRITE_SETT_LINE("CALIBRATE.rf_out_power_up30mhz", (uint32_t*)&CALIBRATE.rf_out_power_up30mhz, SYSMENU_UINT8);
		if(res) SD_WRITE_SETT_LINE("CALIBRATE.rf_out_power_up60mhz", (uint32_t*)&CALIBRATE.rf_out_power_up60mhz, SYSMENU_UINT8);
...
static void SDCOMM_PARSE_SETT_LINE(char* line)
{
	...
	if (strcmp(name, "CALIBRATE.rf_out_power_up30mhz") == 0) CALIBRATE.rf_out_power_up30mhz = (uint8_t)uintval;
	if (strcmp(name, "CALIBRATE.rf_out_power_up60mhz") == 0) CALIBRATE.rf_out_power_up60mhz = (uint8_t)uintval;
