...
static void SDCOMM_EXPORT_SETTINGS_handler(void) {
	...
	if (f_open(&File, (char *)SD_workbuffer_A, FA_CREATE_ALWAYS | FA_WRITE) == FR_OK) {
		...
		if (res) {
			...
			SD_WRITE_SETT_LINE("TRX.FFT_CenterAfterIdle", (uint64_t *)&TRX.FFT_CenterAfterIdle, SYSMENU_BOOLEAN);
...
static void SDCOMM_PARSE_SETTINGS_LINE(char *line) {
	...
	if (strcmp(name, "TRX.FFT_CenterAfterIdle") == 0) {
		TRX.FFT_CenterAfterIdle = bval;
	}
