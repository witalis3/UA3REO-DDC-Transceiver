...
static void SDCOMM_EXPORT_SETT_handler(void)
{
	...
	if (f_open(&File, "wolf.ini", FA_CREATE_ALWAYS | FA_WRITE) == FR_OK)
	{
		...
		if (res)
		{
			...
			SD_WRITE_SETT_LINE("CALIBRATE.AM_DC_RATE", (uint32_t *)&CALIBRATE.AM_DC_RATE, SYSMENU_UINT8);
...
static void SDCOMM_PARSE_SETT_LINE(char *line)
{
	...
	if (strcmp(name, "CALIBRATE.AM_DC_RATE") == 0)
		CALIBRATE.AM_DC_RATE = (uint8_t)uintval;
