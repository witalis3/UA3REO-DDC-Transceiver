...
// NB3
#define NB3_REFERENCE_SIZE (NB_BLOCK_SIZE * 2) // size of the reference buffer
// #define NB3_TAPS 32                                    // filter order
// #define NB3_STEP 0.01f                               // LMS algorithm step
#define NB3_TAPS 32    // filter order
#define NB3_STEP 0.05f // LMS algorithm step
...
typedef struct {
	...
	// NB2 with averaging
	float32_t NB2_d_avgsig;
	float32_t NB2_d_avgmag_nb2;
	// NB3 with LMS
	arm_lms_norm_instance_f32 NB3_lms2_Norm_instance;
	float32_t NB3_lms2_stateF32[NB3_TAPS + 1];
	float32_t NB3_lms2_normCoeff_f32[NB3_TAPS];
	float32_t NB3_lms2_err;
} NB_Instance;
