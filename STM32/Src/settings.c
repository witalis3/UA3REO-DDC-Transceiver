...
char version_string[19] = "3.2.2-dev"; //1.2.3-yymmdd.hhmm (concatinate)
...
void LoadCalibration(bool clear)
{
	...
	if (CALIBRATE.ENDBit != 100 || CALIBRATE.flash_id != CALIB_VERSION || clear || CALIBRATE.csum != calculateCSUM_EEPROM()) // code for checking the firmware in the eeprom, if it does not match, we use the default
	{
		...
		CALIBRATE.SWR_FWD_Calibration_HF = 16.0f;	   //SWR Transormator rate forward
		CALIBRATE.SWR_REF_Calibration_HF = 16.0f;	   //SWR Transormator rate return
		CALIBRATE.SWR_FWD_Calibration_6M = 16.0f;	   //SWR Transormator rate forward
		CALIBRATE.SWR_REF_Calibration_6M = 16.0f;	   //SWR Transormator rate return
		CALIBRATE.SWR_FWD_Calibration_VHF = 16.0f;	   //SWR Transormator rate forward
		CALIBRATE.SWR_REF_Calibration_VHF = 16.0f;	   //SWR Transormator rate return
