...
static void LCD_displayTopButtons(bool redraw) { //вывод верхних кнопок
	...
	LCD_busy = true;
	if (redraw) LCDDriver_Fill_RectWH(LAY_TOPBUTTONS_X1, LAY_TOPBUTTONS_Y1, LAY_TOPBUTTONS_X2, LAY_TOPBUTTONS_Y2, BACKGROUND_COLOR);
	...
	printInfo(LAY_TOPBUTTONS_PRE_X, LAY_TOPBUTTONS_PRE_Y, LAY_TOPBUTTONS_PRE_W, LAY_TOPBUTTONS_PRE_H, "PRE", BACKGROUND_COLOR, COLOR_BUTTON_TEXT, COLOR_BUTTON_INACTIVE_TEXT, TRX.Preamp);
...
static void LCD_displayStatusInfoGUI(void) { //вывод RX/TX и с-метра
	...
	else
	{
		...
		else
		{
			...
			if (TRX.FFT_Zoom == 1) LCDDriver_printText("48kHz", LAY_STATUS_BAR_X_OFFSET + LAY_STATUS_SMETER_WIDTH + LAY_STATUS_LABEL_NOTCH_X_OFFSET, LAY_STATUS_Y_OFFSET+LAY_STATUS_LABELS_OFFSET_Y, LAY_STATUS_LABELS_BW_COLOR, BACKGROUND_COLOR, LAY_STATUS_LABELS_FONT_SIZE);
			if (TRX.FFT_Zoom == 2) LCDDriver_printText("24kHz", LAY_STATUS_BAR_X_OFFSET + LAY_STATUS_SMETER_WIDTH + LAY_STATUS_LABEL_NOTCH_X_OFFSET, LAY_STATUS_Y_OFFSET+LAY_STATUS_LABELS_OFFSET_Y, LAY_STATUS_LABELS_BW_COLOR, BACKGROUND_COLOR, LAY_STATUS_LABELS_FONT_SIZE);
			if (TRX.FFT_Zoom == 4) LCDDriver_printText("12kHz", LAY_STATUS_BAR_X_OFFSET + LAY_STATUS_SMETER_WIDTH + LAY_STATUS_LABEL_NOTCH_X_OFFSET, LAY_STATUS_Y_OFFSET+LAY_STATUS_LABELS_OFFSET_Y, LAY_STATUS_LABELS_BW_COLOR, BACKGROUND_COLOR, LAY_STATUS_LABELS_FONT_SIZE);
			if (TRX.FFT_Zoom == 8) LCDDriver_printText(" 6kHz", LAY_STATUS_BAR_X_OFFSET + LAY_STATUS_SMETER_WIDTH + LAY_STATUS_LABEL_NOTCH_X_OFFSET, LAY_STATUS_Y_OFFSET+LAY_STATUS_LABELS_OFFSET_Y, LAY_STATUS_LABELS_BW_COLOR, BACKGROUND_COLOR, LAY_STATUS_LABELS_FONT_SIZE);
			if (TRX.FFT_Zoom == 16) LCDDriver_printText(" 3kHz", LAY_STATUS_BAR_X_OFFSET + LAY_STATUS_SMETER_WIDTH + LAY_STATUS_LABEL_NOTCH_X_OFFSET, LAY_STATUS_Y_OFFSET+LAY_STATUS_LABELS_OFFSET_Y, LAY_STATUS_LABELS_BW_COLOR, BACKGROUND_COLOR, LAY_STATUS_LABELS_FONT_SIZE);
		}
	}
...
static void LCD_displayStatusInfoBar(void) { //S-метра и прочей информации
	...
	if (TRX_ADC_OTR || TRX_DAC_OTR || TRX_ADC_MAXAMPLITUDE > (powf(2.0f,ADC_BITS)*0.49f) || TRX_ADC_MINAMPLITUDE < -(powf(2.0f,ADC_BITS)*0.49f))
