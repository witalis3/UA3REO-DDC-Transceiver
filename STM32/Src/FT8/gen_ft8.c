...
//		message_state = 0;	
    ...
    switch ( index )
			...
			case 1:   sprintf(message,"%s %s %s", Target_Call,Station_Call,Locator);			//Answer a "CQ"
			break;
			...
			case 4:   sprintf(message,"%s %s R%3i", Target_Call,Station_Call,Target_RSL);	//Send a raport for the - Answer "CQ" case (diffrence from "2" is the "R")
			break;			
	
			case 5:   sprintf(message,"%s %s %s", Target_Call,Station_Call,seventy_three2);	//Send a "73" for the - Answer "CQ" case (diffrence from "3" is the missing "RR")
			break;		
