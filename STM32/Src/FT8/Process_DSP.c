...
void init_DSP(void) {
   ...
   for (int i = 0; i < FFT_SIZE_FT8; ++i) window[i] = ft_blackman_i(i, FFT_SIZE_FT8);
...
float ft_blackman_i(int i, int N) {
  ...
  void extract_power( int offset) {
          ...
          for (int i = 0; i <  FFT_SIZE_FT8 ; i++) window_dsp_buffer[i] = (q15_t) ( (float) dsp_buffer[i + time_sub] * window[i] );
