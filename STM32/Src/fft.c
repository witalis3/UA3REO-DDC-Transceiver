...
static const arm_fir_decimate_instance_f32 FirZoomFFTDecimate[17] =
	{
		...
		{NULL}, // 3
		// 48ksps, 6kHz lowpass
		{.numTaps = 4, .pCoeffs = (float *)(const float[]){0.198273254218889416, 0.298085149879260325, 0.298085149879260325, 0.198273254218889416}, .pState = NULL},
		{NULL}, // 5
		{NULL}, // 6
		{NULL}, // 7
		// 48ksps, 3kHz lowpass
		{.numTaps = 4, .pCoeffs = (float *)(const float[]){0.199820836596682871, 0.272777397353925699, 0.272777397353925699, 0.199820836596682871}, .pState = NULL},
		{NULL}, // 9
		{NULL}, // 10
		{NULL}, // 11
		{NULL}, // 12
