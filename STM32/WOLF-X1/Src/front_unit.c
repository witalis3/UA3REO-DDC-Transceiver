...
const PERIPH_FrontPanel_FuncButton PERIPH_FrontPanel_FuncButtonsList[FUNCBUTTONS_COUNT] = {
    ...
    {.name = "NOTCH",
     ...
     .holdHandler = BUTTONHANDLER_NOTCH_MANUAL,
     .checkBool = (uint32_t *)&TRX.Notch_on_shadow},
    {.name = "ATT",
     .work_in_menu = false,
    ...
    {.name = "AGC",
     ...
     .holdHandler = BUTTONHANDLER_AGC,
     .checkBool = (uint32_t *)&TRX.AGC_shadow},
    ...
    {.name = "PLAY",
     ...
     .holdHandler = BUTTONHANDLER_PLAY,
	...
	if (button->type == FUNIT_CTRL_RIT_XIT) {
		if (TRX.RIT_Enabled) {
			...
			if (TRX_RIT_old != TRX_RIT) {
				...
				LCD_bw_trapez_stripe_pos_new = LCD_bw_trapez_stripe_pos_new + (int16_t)((float32_t)(LAYOUT->BW_TRAPEZ_WIDTH * 0.9f) / 2.0f *
				                                                                        ((float32_t)TRX_RIT / (float32_t)TRX.RIT_INTERVAL));
			...
			if (TRX_XIT_old != TRX_XIT && !TRX_on_TX) {
				...
				uint16_t LCD_bw_trapez_stripe_pos_new = LAYOUT->BW_TRAPEZ_POS_X + LAYOUT->BW_TRAPEZ_WIDTH / 2;
				LCD_bw_trapez_stripe_pos_new = LCD_bw_trapez_stripe_pos_new + (int16_t)((float32_t)(LAYOUT->BW_TRAPEZ_WIDTH * 0.9f) / 2.0f *
				                                                                        ((float32_t)TRX_XIT / (float32_t)TRX.XIT_INTERVAL));
				if (abs(LCD_bw_trapez_stripe_pos_new - LCD_bw_trapez_stripe_pos) > 0) {
					LCD_bw_trapez_stripe_pos = LCD_bw_trapez_stripe_pos_new;
					LCD_UpdateQuery.StatusInfoGUI = true;
				}
			...
			TRX_XIT = 0;
			TRX.IF_Gain =
			    (uint8_t)(CALIBRATE.IF_GAIN_MIN + ((1023.0f - mcp3008_value) * (float32_t)(CALIBRATE.IF_GAIN_MAX - CALIBRATE.IF_GAIN_MIN) / 1023.0f));
		}
	}
